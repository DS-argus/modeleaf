...
- 읽기 영역을 활성 패널에서 tmux 방식으로 제자리 분할하는 깊이 1·최대 4 패널 트리로 구성: 어느 방향으로든 먼저 분할할 수 있고, 비대칭 3패널 두 형태와 2×2 두 방향 모두에 도달할 수 있으며 각 패널은 자체 탭과 활성 문서를 소유
...
| 확대/축소 | `=` / `-` |
| 너비/페이지 맞춤 | `w` / `f` |
| 테마 선택 | `Shift-t` (실시간 미리보기; 선택 항목 저장) |
...
패널 레이아웃은 깊이 1에서 1~4개 패널로 제한됩니다. `Ctrl-b |`와 `Ctrl-b -`는 활성 패널을 제자리에서 분할하며 어느 방향으로든 먼저 분할할 수 있고, 비대칭 3패널 두 형태와 2×2 두 방향 모두에 도달할 수 있습니다. 병렬 분할 요청이나 이미 분할된 밴드에서의 요청은 엄격한 no-op입니다. 새 패널은 활성 패널의 현재 페이지를 자신의 크기에 맞춰(fit-to-page) 엽니다 — 읽던 위치는 그대로 이어지고 확대율은 더 작아진 새 패널에 맞게 조정됩니다. `Ctrl-b o`는 전역 close-others로, 활성 패널만 남기고 다른 모든 패널을 닫습니다.

포커스는 기하학적으로 같은 슬롯으로 건너갑니다. 전체 폭/높이 패널이 분할 밴드로 건너갈 때만 그 밴드에서 마지막으로 포커스했던 슬롯을 사용하고, 기록이 없으면 첫 슬롯을 선택합니다. 경계 이동은 no-op입니다. traffic-light 여백은 왼쪽 위 패널에만 적용됩니다.
44개 액션 전체 목록, 키 토큰 문법, 검증 규칙, 수치 범위, 기본 TOML은 [한국어 설정 가이드](CONFIG.md)에서 확인할 수 있습니다.
