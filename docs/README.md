...
패널 레이아웃은 깊이 1에서 1~4개 패널로 제한됩니다. `Ctrl-b |`와 `Ctrl-b -`는 활성 패널을 제자리에서 분할하며 어느 방향으로든 먼저 분할할 수 있고, 비대칭 3패널 두 형태와 2×2 두 방향 모두에 도달할 수 있습니다. 병렬 분할 요청이나 이미 분할된 밴드에서의 요청은 엄격한 no-op입니다. `Ctrl-b o`는 전역 close-others로, 활성 패널만 남기고 다른 모든 패널을 닫습니다.
