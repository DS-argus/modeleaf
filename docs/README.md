...
- 암호로 보호된 로컬 PDF 열기: 네이티브 보안 입력창을 사용하며 암호는 저장하지 않음
- 명령 팔레트와 7가지 테마
...
본문을 떠나지 않고 인용된 참고문헌을 확인합니다.

- **활성화:** `Shift+C`로 켜고 끕니다. 기본값은 OFF이며 선택은 저장됩니다.
- **사용:** `f`로 활성 pane 안의 링크 힌트를 표시한 뒤 인용 링크를 선택합니다. 힌트 입력 중 `f`를 다시 누르면 `Esc`처럼 닫힙니다(일부 글자를 입력한 뒤에도 동일). 힌트 label에는 `f`가 포함되지 않습니다.
- **탐색:** `h` / `l` 또는 `Tab` / `Shift+Tab`으로 문헌 전환, `Enter`로 참고문헌 이동, `Shift+Enter`로 Google Scholar 검색, `Esc`로 닫기.
- **대표 지원 형태:** AI 분야 학회 논문에서 흔히 쓰이는 인용 형태를 우선 지원합니다. 예: `[1]`, `[1,3]`, `[1–3]`, `(1; 2)`, `(Author, 2020)`, `(Author, 2020a,b)`, `[HKR16; Pat+23]`.

PDF에 기존 내부 인용 링크가 있어야 합니다. 문서의 링크 구조와 텍스트·레이아웃에 따라 일부 문헌은 표시되지 않거나 부정확하게 추출될 수 있습니다. 위첨자 인용과 링크 없는 인용은 지원 대상에서 제외합니다.

<details>
<summary>인용 미리보기 스크린샷 보기</summary>

![Counterfactual Structural Causal Bandits의 Pearl and Mackenzie 2018 인용 미리보기](../Assets/CitationPreview.png)
...
`y`는 PDF 전체 경로를 표시하고, `yy`는 경로를 복사한 뒤 `copied!`를 덧붙입니다. 입력할 때마다 표시 시간이 3초로 갱신되며, 연속 키 입력 제한은 기본 400ms입니다. 긴 경로는 가운데를 생략하고 전체 경로는 tooltip으로 제공합니다.

외부 URL 힌트를 선택하면 목적지 URL을 표시하며 바로 열지는 않습니다. Enter 한 번으로 열고 Esc로 닫습니다. 키를 누르고 있을 때 발생하는 반복 입력은 링크를 열지 않습니다. 문서 내부 이동에는 이 확인을 적용하지 않습니다. `[links]`의 `skip_external_link_hint_confirmation = true`로 외부 힌트 확인을 생략할 수 있습니다.

상태바는 기존 최소 창 크기인 480 × 360pt까지 한 줄로 대응합니다. 일시적인 경로·키 입력 표시와 최소 검색 정보를 우선하며, 공간이 부족하면 버전 → 도움말 → 배율 → Fit 배지 → 페이지 순서로 기본 정보를 숨깁니다. 일반 안내와 업데이트는 온전히 들어갈 때만 표시하고, 오류 문구가 들어가지 않으면 `Error` 버튼에서 전체 내용을 확인할 수 있습니다.
