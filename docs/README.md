...
외부 URL 힌트를 선택하면 목적지 URL을 표시하며 바로 열지는 않습니다. 첫 Enter로 선택을 확인하고 다음 Enter로 엽니다. Esc로 닫을 수 있으며 문서 내부 이동에는 이 확인을 적용하지 않습니다. `[links]`의 `skip_external_link_hint_confirmation = true`로 외부 힌트 확인을 생략할 수 있습니다.
