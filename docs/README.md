...
패널 구조는 단일 패널, 상하 2개, 좌우 2개, 비대칭 3개, 2×2로 제한됩니다. `Ctrl-Space |`는 열이 하나일 때(상하 2개인 열 포함)에만 좌우 분할을 만들고, `Ctrl-Space -`는 활성 단일 패널 열만 상하로 분할합니다. 이 제한을 넘는 분할은 아무 동작도 하지 않습니다. `Ctrl-Space o`는 전역 close-others로, 활성 패널만 남기고 다른 모든 패널을 닫습니다.

`Ctrl-h`와 `Ctrl-l`은 열 사이를 이동합니다. 목적지 열이 상하로 나뉘어 있으면 Modeleaf는 그 열에서 마지막으로 포커스했던 행을 tmux처럼 기억하며, 아직 방문하지 않은 열은 위 패널을 선택합니다. `Ctrl-j`와 `Ctrl-k`는 활성 열 안에서만 이동합니다. traffic-light 여백은 왼쪽 위 패널에만 적용됩니다.
